...
| [CISA KEV Catalog](https://www.cisa.gov/known-exploited-vulnerabilities-catalog) | Motivates optional `kev` finding flag ([ADR-003](../decisions/ADR-003-kev-multiplier.md)); catalog is not fetched by this CLI |
...
Future work may optionally ingest KEV membership from the live catalog as an additional factor; that would need a new ADR. Today only the explicit finding `kev` boolean is supported.
