...
5. **CLI** — `vrscore` / `vuln-reachability` prints a ranked table, JSON, JSONL, CSV, SARIF 2.1.0, HTML, or Markdown (`cli.py`, `sarif.py`, `html_report.py`, `markdown_report.py`). Optional finding `epss` is applied in scoring ([ADR-004](../decisions/ADR-004-epss-factor.md)).
...
| `markdown_report.py` | GitHub-flavored Markdown table report |
